...
**3. Plan, Act, Audit.** Three separate passes with separate actors. The agent that built a thing never signs off on it. Audit is not a formality, it has authority to send work back. **An autonomous run stops at the end of Phase 1 and waits for a human.** It does not approve its own plan. A run that approves its own plan has deleted the one checkpoint that catches a wrong direction at Phase 1 rather than at Phase 9, which is where the cost of being wrong is twenty times higher.
...
**HARD STOP. Get plan approval from the user before Act.**

This is a blocking gate, not an instruction to note and move past. Present the plan, the crew table, the budget preflight from `kaga-budget`, and stop. Wait for a human answer.

Autonomous mode does not exempt a run from this. If nobody is there to answer, the run ends here with the plan written to `docs/PLAN.md` and says so. A plan approved by the thing that wrote it is not a checkpoint.
...
Run independent phases in parallel **only when the work is latency-bound and cap headroom exists**. P3, P5, and P6 can overlap on the dependency graph.

Parallelism does not reduce token cost, it spends the same tokens faster. When the 5-hour Pro window is short, or the API budget is tight, run them in sequence: identical total cost, spread across windows instead of exhausting one. Check `kaga-budget` before choosing.
